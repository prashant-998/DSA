...
public:
    int minOperations(vector<int>& nums, int x) 
    {
        ...
        if(ans==-1) 
        {
        // if after whole traversal , ans==-1 (initialized value) --> we didn't find any valid answer -->                hence return -1
            return -1;
        }
        // else return ( total array length) - (maximum length of middle portion )
        else 
        {
